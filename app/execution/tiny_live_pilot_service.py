    ...
    def _checks(self, *, flags: Any, context: dict[str, Any], prepared: "PilotPreparation", mode: str, confirm: str) -> list[dict[str, Any]]:
        ...
        return [
            ...
            self._check(
                ...
                mode != "swap" or readiness.get("live_review_ready") is True or (tx_sim.get("transaction_simulation_passed") is True and self._int(readiness.get("blocked_check_count")) == 0),
                "Live readiness checklist must have no hard blockers and the smoke transaction simulation must pass before swap mode.",
                "Live readiness checklist is ready or the simulated smoke swap has no hard readiness blockers.",
            ),
            self._check("transaction_simulation_passed", mode != "swap" or tx_sim.get("transaction_simulation_passed") is True, "Transaction simulation must pass before swap mode.", "Transaction simulation passed or is not required for this mode."),
